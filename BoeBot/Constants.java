public class Constants
{
    // Infrarood sensor
    public static final int IFR_PIN = 9;
    public static final int[] IFR_HWID = new int[]{ 1200, 600, 600, 600 };
    
    // Servo Engines
    public static final int SERVO_LEFT = 12;
    public static final int SERVO_RIGHT = 13;   
    
    // Ultrasoon sensor
    public static final int ULTRASOON_IN_PIN = 3;
    public static final int ULTRASOON_OUT_PIN = 4;
    
    public static final int ULTRASOON_DISTANCE = 1000;
    
    public static final int LED_LEFT_PIN = 15;
    public static final int LED_RIGHT_PIN = 0;
    
    public static final int WHISKER_LEFT_PIN = 5;
    public static final int WHISKER_RIGHT_PIN = 6;
    
    public static final int BUZZER_PIN = 7;
    
    public static final int LIGHT_SENSOR_LEFT = 0;
    public static final int LIGHT_SENSOR_CENTER = 1;
    public static final int LIGHT_SENSOR_RIGHT = 2;
    
    public static final char BEGIN_END_PATH_CHAR = ';';
    public static final char BEGIN_END_COORDINATE_CHAR = ':';
    
    public static final int TRIGGER_BLACK = 500;
}
